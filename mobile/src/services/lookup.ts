...
export async function lookupIncident(reference: string): Promise<LookupResult> {
  ...
  const { data } = await supabase
    .from('incidents')
    .select('id, case_number, incident_type, description, location, police_status, created_at')
...
export async function lookupByIdNumber(idNumber: string): Promise<LookupResult> {
  const supabase = getSupabase();
  const { data: rider } = await supabase
    .from('riders')
    .select(
  ...
  if (!rider) return { type: 'not_found', identifier: idNumber };

  const motorcycle = rider.motorcycle_id ? await fetchMotorcycle(rider.motorcycle_id) : null;
  const compliance = deriveCompliance(rider, motorcycle);

  return { type: 'rider', data: { ...rider, motorcycle, compliance } as RiderLookup };
}
