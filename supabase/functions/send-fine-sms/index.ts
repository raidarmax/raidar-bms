...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const dueDate =
      due_date_override ||
      new Date(Date.now() + 14 * 24 * 60 * 60 * 1000).toLocaleDateString("en-KE");

    const templateVarsRider = {
      fine_reference,
      fine_amount: String(fine_amount ?? ""),
      offence_name: offence_name || "traffic offence",
      due_date: dueDate,
    ...
    const templateVarsOwner = {
      fine_reference,
      fine_amount: String(fine_amount ?? ""),
      rider_name: rider_name || "the rider",
      offence_name: offence_name || "traffic offence",
      due_date: dueDate,
      station_name: station_name || "",
    };

    const defaultRiderIssuance =
    ...
    const defaultRiderReminder =
      'BMS FINAL REMINDER: Fine {fine_reference} of KES {fine_amount} for "{offence_name}" is OVERDUE. Settle immediately at {station_name} to avoid further penalties and possible prosecution.';
    const defaultOwnerReminder =
      'BMS FINAL REMINDER: Fine {fine_reference} of KES {fine_amount} issued to {rider_name} for "{offence_name}" is OVERDUE. Please facilitate immediate payment at {station_name}.';

    const riderTemplate = isReminder
      ? settingsMap["fine_rider_reminder"] || defaultRiderReminder
      : settingsMap["fine_rider_message"] || defaultRiderIssuance;
    const ownerTemplate = isReminder
      ? settingsMap["fine_owner_reminder"] || defaultOwnerReminder
      : settingsMap["fine_owner_message"] || defaultOwnerIssuance;

    const riderMessage = applyTemplate(riderTemplate, templateVarsRider);
    const ownerMessage = applyTemplate(ownerTemplate, templateVarsOwner);
    ...
    async function sendSms(phone: string, message: string, recipientType: string) {
      ...
      await supabase.from("fine_sms_logs").insert({
        ...
        recipient_type: isReminder ? `${recipientType}_reminder` : recipientType,
    ...
    if (isReminder && fine_id) {
      await supabase
        .from("fines")
        .update({
          last_reminder_sent_at: new Date().toISOString(),
          reminder_count: (body.current_reminder_count ?? 0) + 1,
        })
        .eq("id", fine_id);
    }

    return new Response(
      JSON.stringify({ success: true, results, reminder: isReminder }),
      { status: 200, headers: { ...corsHeaders, "Content-Type": "application/json" } }
    );
  } catch (err) {
